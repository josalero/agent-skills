...
class InstallTests(unittest.TestCase):
    ...
    def test_install_java_backend_pack_opencode_active_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            dest = Path(tmp) / "project"
            dest.mkdir()
            result = install_pack(ROOT, "java-backend-pack", "opencode", dest, active_only=True)
            self.assertIn("java-core-engineering", result.installed)
            self.assertTrue((dest / ".opencode/skills/java-core-engineering/SKILL.md").exists())
            self.assertFalse((dest / ".opencode/rules").exists())
