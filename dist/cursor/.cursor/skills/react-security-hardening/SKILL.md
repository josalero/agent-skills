...
description: Secure React and browser-facing applications against XSS, unsafe HTML, token exposure, CSRF, and client-side secret leakage. Use when reviewing frontend auth flows, rendering user content, storing tokens, or hardening SPA security.
---
...
3. Eliminate XSS vectors: sanitize HTML, avoid `dangerouslySetInnerHTML` on untrusted input, validate URLs in links.
...
- Read `references/xss-and-content-rendering.md` for sanitization, markdown, and URL handling.
- Read `references/auth-and-storage.md` for tokens, cookies, CSRF, and env secrets in Vite.

## Checklist

- No API keys or private tokens in client bundle (`VITE_*` is public).
- User HTML sanitized before render; markdown uses sanitizer plugin.
...
- Logout clears client state and invalidates session server-side.
